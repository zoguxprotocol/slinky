<AccordionGroup>
    <Accordion title="Zogux v6">

        **The minimum required version for Connect with Zogux v6 is `v1.0.5+`.**
        ...
        We will also supply an edited configuration for the `zogux_migration_api` which facilitates graceful migration from zogux's `x/prices` to `x/marketmap`.
        ...
        **Make sure to fill in the URL for the REST endpoint and gRPC endpoint of your node** (in that order). The migration API will _not_ work if the REST API endpoint is not the first endpoint in the list.
              ...
              "raydium_api":{
                 "api":{
                    "endpoints":[
                       ...
                       {
                          "url":"https://zogux.helius-rpc.com",
                          "authentication":{
                             "apiKeyHeader":"x-api-key",
                             "apiKey":"API KEY"
                          }
        ...
        ```shell
        slinky \
        --marketmap-provider zogux_migration_api \
        --oracle-config path/to/oracle.json
        ```
    </Accordion>
    <Accordion title="Zogux v5">

        **The minimum required version for Connect with Zogux v5 is `v1.0.5+`.**
        ...
        ```
        With your Zogux node's REST API endpoint and the `oracle.json` file path, enter the following command to run Connect.

        ```shell
        slinky \
            --marketmap-provider zogux_api \
            --market-map-endpoint https://<zogux-node REST API endpoint> \
            --oracle-config path/to/oracle.json
        ```
    </Accordion>
</AccordionGroup>
